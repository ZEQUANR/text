...
type Node struct {
	...
	// Ssssss holds the value of the "ssssss" field.
	Ssssss int `json:"ssssss,omitempty"`
...
func (*Node) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case node.FieldID, node.FieldValue, node.FieldSsssss:
...
func (n *Node) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case node.FieldSsssss:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field ssssss", values[i])
			} else if value.Valid {
				n.Ssssss = int(value.Int64)
			}
		default:
...
func (n *Node) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", n.Ssssss))
	builder.WriteByte(')')
